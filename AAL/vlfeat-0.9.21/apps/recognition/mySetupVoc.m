function imdb = mySetupVoc(datasetDir, varargin)
...
datasetDir = 'data/VOCdevkit';

imdb.images.id = [] ;
% unit8()  MATLAB 中数据类型（类）为 uint8 的变量存储为 1 个字节（8 位）的无符号整数
...
% Map对象 允许您使用对应键检索值的数据结构体
map = containers.Map() ;
j = 0 ;
% numel 数组元素的数目
for si = 1:numel(imdb.meta.sets)
    % 当前是训练集(si=0)、验证集(si =1 )以及测试集(si = 2)
  setName = imdb.meta.sets{si} ;
    % 获取train.txt路径，里面是图片id
  annoPath = fullfile(datasetDir, ['VOC' opts.edition], ...
    'ImageSets', 'Main', ...
    [setName '.txt']) ;
  % mfilename 当前正在运行的代码的文件名
  fprintf('%s: reading %s\n', mfilename, annoPath) ;
  %textread 读取文本文件中的数据；写入多个输出; 
  % 这里通过运行代码发现 names就是对应的image id ，而labels都是0， 其中二者shape一样。
  [names,labels] = textread(annoPath, '%s %f') ;
  
  for i=1:length(names)
    j = j + 1 ;
    % map函数没有搞懂？？
    % 这里的names{i} 的数值类型是“char”
    map(names{i}) = j ;
    imdb.images.id(j) = j ;
