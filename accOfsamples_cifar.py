...
def order_output(target_lsa,select_amount):
    ...
    while len(selected_lst) < select_amount:
        min_lsa=max(tmp_lsa_lst)
        selected_lst.append(target_lsa.index(min_lsa))
        lsa_lst.append(min_lsa)
        tmp_lsa_lst.remove(min_lsa)
...
def fetch_our_entropy(model, x_target):
    ...
    for i in range(len(act_layers)):
        act=act_layers[i]
        entropy = cal_entropy(act)      
        entropy_lst.append(entropy)
        
    return entropy_lst

#先随机，然后再用我们的度量
def select_after_rondom(model, x_target):
...
if __name__ == "__main__":
    ...
    #model=load_model(model_path)
    #model.layers[1].name
    #fgsm  cw-l2 bim-b jsma 
    #bim-a fgsm已跑
    #measurelst=['conditional','random'] 
    #for attack in ['fgsm','jsma','bim-a','bim-b','cw-l2']:   
        #createdataset(attack,9)
    #for attack in ['bim-b']:#'jsma','bim-a',,'cw-l2']:   
    for attack in ['fgsm','jsma','bim-a','bim-b','cw-l2','shear','rotation','brightness','contrast','scale','translation']: 
    #for attack in ['fgsm','jsma','bim-a','scale','brightness','contrast','bim-b']:
                   #'rotation','translation','shear','brightness','contrast','scale']:
        #createdataset(attack,9)
        #for attack in ['fgsm','jsma','bim-a','bim-b','cw-l2']:0.9076  0.8230
        model=load_model(model_path)
    ...
    #print('Test Loss: %.4f' % score[0])
        ...
        for measure in ['my_optimize']:
        #for measure in ['my']:
        #,'my','random','lsa','dsa','conditional']:
            #x_target=np.load('./adv/data/mnist/Adv_mnist_'+attack+'.npy') 
            #x_target=np.load('./adv/data/mnist/mnist_'+attack+'.npy')
            layer_names= ['activation_40']
            #resultfilename = './result/adpt_cifar_'+attack+'.txt'
            resultfilename = './result/cifar_accofsamples.txt'
            #resultfilename = './result/mnist_my_random_'+attack+'.txt'
            #origin_acc=0
            result_to_write='' 
            result_to_write+=attack+':\n'
            for selectsize in [100,300,500,1000]:
                ...
                score = model.evaluate(x_select, y_select,verbose=0) 
                print(attack)
                result_to_write+=str(selectsize)+':\n'
                result_to_write+=str(round(score[1],4))+':\n'
                print(selectsize)
                print(score[1])
                continue          
                ...
                #endtime = time.time()
                #dtime = endtime - starttime
                #result_to_write+='selected index list:\n'+str(indexlst)
